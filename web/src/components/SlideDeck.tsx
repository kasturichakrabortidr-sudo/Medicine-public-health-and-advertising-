...
import LeaveOneOut from "./LeaveOneOut";
...
export default function SlideDeck({ deck }: { deck: DeckPayload }) {
  ...
  function downloadCsv() {
    const header = "n,title,doi,url,validated_via";
    const rows = (deck.references || []).map((r) => {
      const cells = [r.n, r.title, r.doi || "", r.url, r.validated_via || ""].map((cell) => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "validated-references.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
...
function buildSlides(deck: DeckPayload) {
  ...
  return [
    ...
    },
    {
      ...
      node: (
        <>
          ...
          <p>
            Open-access and paywalled indexed journals (Europe PMC including an
            OA-only harvest, OpenAlex including OA-only and guideline-title
            harvests, Crossref, PubMed/MEDLINE plus MeSH, Semantic Scholar,
            Unpaywall OA flags), Cochrane Database records, DOAJ, preprint
            indexes, national and
            international guidelines (ESC, AHA, ACC, NICE NG106/TA388, KDIGO,
            CCS/CHFS, NHFA/CSANZ, CSI, ICMR / NPCDCS / CDSCO when retrieved),
            WHO IRIS, WHO publications, WHO PEN, UN-system institutions on
            OpenAlex (WHO, PAHO, UNICEF, UNAIDS, UNDP, UNFPA, UNESCO, ILO, UNEP,
            UNHCR, FAO, WFP, IOM, UNODC, UN Women, UN-Habitat, ITU, UN DESA,
            OHCHR, World Bank), NGOs, and ClinicalTrials.gov (including
            completed studies with results). Official URL seeds add
            WHO regional CVD/NCD pages (SEARO, Europe, Africa, EMRO, Western
            Pacific), SDG 3, UNICEF NCD, ITU, OHCHR right-to-health, WHO UHC
            and primary-care hubs, and India
            ministry / ICMR / CSI / CDSCO portals when the brief matches. Paywalled papers
    ...
    },
    {
      ...
      node: (
        <>
          ...
          <ForestPlot rows={deck.forest} pooled={deck.quantitative.pooled_effect} />
        </>
      ),
    },
    {
      ...
      node: (
        <>
          <h2>13 · Inverse-variance summary and funnel</h2>
          <p>
            A fixed-effect inverse-variance summary of the named-trial 95% CIs
            already on the forest plot. This is a transparency calculation — not
            a de-novo full-text meta-analysis. Missing numbers are never filled
            in.
            {deck.quantitative.pooled_effect
              ? ` Pooled ${deck.quantitative.pooled_effect.metric} ${deck.quantitative.pooled_effect.value} (${deck.quantitative.pooled_effect.ci_low}–${deck.quantitative.pooled_effect.ci_high}); I² ${deck.quantitative.pooled_effect.i_squared}% from ${deck.quantitative.pooled_effect.n_trials} trials.`
              : ""}
            {deck.quantitative.pooled_effect?.random_effect
              ? ` Random-effects DL HR ${deck.quantitative.pooled_effect.random_effect.value} (${deck.quantitative.pooled_effect.random_effect.ci_low}–${deck.quantitative.pooled_effect.random_effect.ci_high}); τ² ${deck.quantitative.pooled_effect.random_effect.tau_squared}.`
              : ""}
          </p>
          <FunnelPlot pooled={deck.quantitative.pooled_effect} />
        </>
      ),
    },
    {
      ...
      node: (
        <>
          <h2>14 · DerSimonian-Laird random-effects</h2>
          <p>
            When I² is high, a single fixed-effect diamond overstates precision.
            The random-effects row uses the same parsed HR CIs — never invented
            numbers — and inflates variance by τ².
          </p>
          {deck.quantitative.pooled_effect?.random_effect ? (
            <div className="kpi-row">
              <div className="kpi">
                <strong>{deck.quantitative.pooled_effect.value}</strong>
                fixed-effect HR ({deck.quantitative.pooled_effect.ci_low}–
                {deck.quantitative.pooled_effect.ci_high})
              </div>
              <div className="kpi">
                <strong>{deck.quantitative.pooled_effect.random_effect.value}</strong>
                random-effects HR ({deck.quantitative.pooled_effect.random_effect.ci_low}–
                {deck.quantitative.pooled_effect.random_effect.ci_high})
              </div>
              <div className="kpi">
                <strong>{deck.quantitative.pooled_effect.i_squared}%</strong>I²
              </div>
              <div className="kpi">
                <strong>{deck.quantitative.pooled_effect.random_effect.tau_squared}</strong>τ²
              </div>
            </div>
          ) : (
            <p>Random-effects needs at least two parsed HR CIs.</p>
          )}
        </>
      ),
    },
    {
      ...
      node: (
        <>
          <h2>15 · Leave-one-out sensitivity</h2>
          <p>
            Each bar reconstitutes the fixed-effect IV after dropping one named
            trial. Use this to see whether one paper is carrying the pooled HR.
          </p>
          <LeaveOneOut pooled={deck.quantitative.pooled_effect} />
        </>
      ),
    },
    {
      ...
      node: (
        <>
          <h2>16 · Evidence-certainty table (design bands)</h2>
          <p>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>17 · Pivotal trials and guidelines carousel</h2>
          <p>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>18 · International and national guidelines</h2>
          <table className="table">
            <thead>
              <tr>
                <th>#</th>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>19 · UN branches and non-profit references</h2>
          <p>
            ...
            UNODC, UNFPA, UN Women, UN DESA, UNICEF NCD, ITU, OHCHR), WHO
            regional CVD/NCD pages
            (SEARO, Europe, Africa, EMRO, Western Pacific), the WHO hypertension
            guideline, WHO PEN, WHO UHC, PAHO HEARTS, the World Bank
            NCD brief, ACC, KDIGO, NICE TA388, NHLBI, and India NHM / MoHFW /
            ICMR / NPCDCS / CSI / CDSCO portals when the
            market matches.
          </p>
          {agencyData.length ? (
    ...
    },
    {
      ...
      node: (
        <>
          <h2>20 · Where the included evidence sits</h2>
          <p>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>23 · Insights for {deck.insights.cohort}</h2>
          <div className="grid-2" style={{ padding: 0 }}>
            <div>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>24 · Novel angles and evidence gaps</h2>
    ...
    },
    {
      ...
      node: (
        <>
          <h2>25 · Ready to drop into a campaign evidence section</h2>
          <ol>
    ...
    },
    {
      ...
      node: (
        <>
          <h2 id="ref-1">26 · Numbered, registry-validated references</h2>
