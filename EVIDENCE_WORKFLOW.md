...
abstract enrichment are the main calendar win. Pipeline version 2.0.0.
...
- UN-system sources: WHO publications and IRIS; OpenAlex institution filters for
  WHO (and regional offices including EMRO and WPRO), PAHO, UNICEF, UNAIDS, UNDP, UNFPA, UNESCO, ILO, UNEP,
...
- Crossref-confirmed DOI seeds for HFrEF briefs: PARADIGM-HF, PIONEER-HF, DAPA-HF,
  ...
  patient-voice interviews), plus TRANSFORM-HF, the 2024 ACC HFrEF expert
  consensus pathway, and the RELIEVE-HF open-label cohort
...
`references.csv`, `claim-frequency.csv`, `forest-effects.csv`,
`leave-one-out.csv`, and
`evidence-campaign-deck.pptx` (numbered slides with frequency bars and a forest
plot drawn from parsed CIs, including the inverse-variance diamond and the
DerSimonian-Laird diamond) next to
...
1. **Quantitative frequency analysis** — unique included records supporting each coded claim,
   ...
   inverse-variance summary, a DerSimonian-Laird random-effects summary, a
   leave-one-out table, and a funnel (precision vs ln HR) are calculated from
