...
abstract enrichment are the main calendar win. Pipeline version 1.8.0.

## What it searches

- Open-access and indexed/paywalled journals (Europe PMC, OpenAlex, Crossref, PubMed, Semantic Scholar, DOAJ)
- Dedicated OpenAlex OA-only harvest (`open_access.is_oa:true`) in parallel with mixed OA/paywalled search
- Unpaywall OA flags for records that already have a validated DOI
- Cochrane Database of Systematic Reviews (journal-filtered Europe PMC) plus OpenAlex `type:review`
- Europe PMC preprint index (SRC:PPR: medRxiv/bioRxiv) — still Crossref-validated if a DOI exists
- International and national guidelines (ESC, ACC/AHA/HFSA, NICE; ICMR / NPCDCS when retrieved)
...
- Official HTTP-200 UN-branch pages this run: UNAIDS, FAO nutrition, ILO OSH,
  ...
  UN Women, UN DESA, plus the UN SDG 3 landing page
- WHO regional CVD/NCD pages now include SEARO, Europe, Africa, Eastern
  Mediterranean (EMRO), and Western Pacific (WPRO)
- NGO and society references (World Heart Federation, ESC, AHA, ACC guideline hub, KDIGO)
- ClinicalTrials.gov
- Crossref-confirmed DOI seeds for HFrEF briefs: PARADIGM-HF, PIONEER-HF, DAPA-HF,
  ...
  TOPCAT, PARADISE-MI, COAPT, ADVOR, CHAMPION, ESC 2021 + 2023 update, 2022 AHA/ACC/HFSA,
...
- Official URL seeds when the brief matches (HTTP 200 required): NICE NG106,
  WHO HEARTS (publications page, ISBN record, IRIS handle, team-based-care module),
  WHO CVD / NCD / hypertension fact sheets, WHO Global NCD Action Plan, WHO
  adult hypertension guideline, WHO SEARO / Europe / Africa CVD and NCD pages,
  UN SDG 3, ESC HF guideline page, PAHO HEARTS and NCD topic pages, World Bank
  NCD brief, World Heart Federation heart-failure page, and — when the market is
  India — MoHFW, National Health Mission, ICMR, and NPCDCS portals
...
`references.csv`, `claim-frequency.csv`, and
`evidence-campaign-deck.pptx` (numbered slides with frequency bars and a forest
plot drawn from parsed CIs) next to `literature-deck.json`.
...
1. **Quantitative frequency analysis** — unique included records supporting each coded claim,
   ...
   Forest plots keep one primary parsed CI per trial acronym.
