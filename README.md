...
Then open `/deck` for the numbered visual deck (forest plot, GRADE-style table,
claim heatmap, PRISMA flow, IPA carousel, references). Option 4 in
`python start.py` runs the same pipeline. Live Netlify briefs go to `/run`
(background function + Blobs).
