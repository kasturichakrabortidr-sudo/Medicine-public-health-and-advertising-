...
the next brief. Records live as JSON under data/projects, never in the demo pack.
"""
...
from pathlib import Path  # used in save payload
...
def upsert_ongoing(pack: dict) -> dict:
    ...
    for row in list_projects():
        ...
        if row.get("brand") == brand and (row.get("source") or "") == source:
            ...
            return save_project({"id": row["id"], "status": "ongoing", "pack": pack, "title": existing.get("title")})
