...
def _iv_summary(weights: list[float], logs: list[float]) -> dict:
    """Inverse-variance mean on the log-HR scale. Weights must be 1/variance."""
    total_w = sum(weights)
...
def pooled_effects(forest: list[dict]) -> dict | None:
    ...
    Fixed-effect IV is always reported. When two or more HR CIs are present,
    a DerSimonian-Laird random-effects estimate and leave-one-out fixed-effect
    rows are added. This is a transparency calculation on the forest rows, not
    a de-novo full-text meta-analysis. Missing CIs are never filled in.
    """
    weights: list[float] = []
    logs: list[float] = []
    funnel: list[dict] = []
    ...
    if not weights:
        return None
    fixed = _iv_summary(weights, logs)
    k = len(weights)
    total_w = fixed["total_w"]
    sum_w2 = sum(w * w for w in weights)
    denom = total_w - (sum_w2 / total_w) if total_w else 0.0
    tau2 = 0.0
    if denom > 0 and k > 1:
        tau2 = max(0.0, (fixed["q_raw"] - (k - 1)) / denom)
    random = None
    if k > 1:
        re_weights = [1.0 / ((1.0 / w) + tau2) for w in weights]
        re = _iv_summary(re_weights, logs)
        random = {
            "method": (
                "DerSimonian-Laird random-effects inverse-variance of the same "
                "parsed named-trial HR 95% CIs. Shown because I² can be high; "
                "still not a de-novo full-text meta-analysis."
            ),
            "n_trials": k,
            "metric": "HR",
            "value": re["value"],
    ...
    leave_one_out = []
    if k > 1:
        for i, point in enumerate(funnel):
            w2 = weights[:i] + weights[i + 1 :]
            l2 = logs[:i] + logs[i + 1 :]
            omit = _iv_summary(w2, l2)
            leave_one_out.append(
                {
                    "omit": point.get("label"),
                    "citation_id": point.get("citation_id"),
                    "fixed_value": omit["value"],
                    "fixed_ci_low": omit["ci_low"],
                    "fixed_ci_high": omit["ci_high"],
                }
            )
    return {
        "method": (
