...
UN_INSTITUTIONS = [
    ...
    ("I4210105726", "WHO EMRO", "un_agency"),
    ("I4210150475", "WHO WPRO", "un_agency"),
...
ANCHOR_DOI_SEEDS = {
    "heart failure": [
        ...
        "10.1056/NEJMoa1805374",  # MITRA-FR
        "10.1016/S0140-6736(21)01754-2",  # GUIDE-HF
        "10.1016/j.cjca.2021.01.017",  # CCS/CHFS 2021 HFrEF update
        "10.1016/j.hlc.2018.06.1042",  # NHFA / CSANZ 2018 HF guidelines
        "10.1002/ejhf.2115",  # Universal definition and classification of HF
        ...
        "10.1371/journal.pone.0310600",  # qualitative: lived experience, Ethiopia (PLOS)
        "10.1097/SPC.0000000000000405",  # qualitative synthesis: breathlessness
        "10.1001/jama.2022.23924",  # TRANSFORM-HF
        "10.1016/j.jacc.2023.12.024",  # 2024 ACC Expert Consensus Decision Pathway HFrEF
        "10.1002/ejhf.3215",  # RELIEVE-HF open-label cohort
        "10.1161/CIRCHEARTFAILURE.124.011874",  # family-caregiver support in advanced HF
    ],
}
...
def pubmed_mesh(mesh_term: str, extra: str, page_size: int = 6) -> list[EvidenceRecord]:
    """MEDLINE MeSH-constrained harvest; abstracts filled later via Europe PMC."""
    extra = extra.strip() or "guideline OR trial"
    query = f'("{mesh_term}"[MeSH]) AND ({extra})'
    rows = pubmed(query, page_size)
    for rec in rows:
        rec.source_connector = "pubmed_mesh"
    return rows
...
def europe_pmc_oa(query: str, page_size: int = 8) -> list[EvidenceRecord]:
    """Europe PMC records flagged open access (OPEN_ACCESS:Y)."""
    q = f"({query}) AND OPEN_ACCESS:Y"
    rows = europe_pmc(q, page_size)
    for rec in rows:
        rec.source_connector = "europe_pmc_oa"
        rec.source_family = "open_access"
        rec.is_oa = True
    return rows


def openalex_guidelines(search: str, page_size: int = 6) -> list[EvidenceRecord]:
    """OpenAlex works whose titles mention guidelines."""
    rows = openalex(f"{search} guideline", page_size)
    for rec in rows:
        rec.source_connector = "openalex_guidelines"
    return rows
...
def clinical_trials(
    query: str, page_size: int = 6, *, completed_with_results: bool = False
) -> list[EvidenceRecord]:
    ...
    for study in data.get("studies") or []:
        ...
        connector = "clinicaltrials_results" if completed_with_results else "clinicaltrials"
        out.append(
            EvidenceRecord(
                key=nct,
                title=strip_text(title),
...
def un_cvd_official_seeds() -> list[EvidenceRecord]:
    ...
    return [
        ...
        official_document(
            ...
            family="un_agency",
            abstract=(
                "UNICEF NCD programme page covering prevention of cardiovascular and "
                "other noncommunicable diseases across the life course."
            ),
        ),
        official_document(
            key="itu:home",
            ...
            body="ITU",
            family="un_agency",
            abstract="ITU official site — UN specialized agency for digital connectivity, including digital-health mandate.",
        ),
        official_document(
            key="ohchr:health",
            ...
            body="OHCHR",
            family="un_agency",
            abstract="UN Office of the High Commissioner for Human Rights topic page on the right to health.",
        ),
        official_document(
            key="un:global-health",
            title="United Nations — Global issues: health",
            url="https://www.un.org/en/global-issues/health",
            year=2024,
            body="United Nations",
            family="un_agency",
            abstract="UN global-issues health hub covering UHC, NCDs, and the health-related SDGs.",
        ),
        official_document(
            key="unhcr:public-health",
            title="UNHCR — Public health",
            url="https://www.unhcr.org/what-we-do/protect-human-rights/public-health",
            year=2024,
            body="UNHCR",
            family="un_agency",
            abstract="UNHCR public-health programme page — refugee health and NCD care in displacement.",
        ),
        official_document(
            key="iom:migration-health",
            title="IOM — Migration health",
            url="https://www.iom.int/migration-health",
            year=2024,
            body="International Organization for Migration",
            family="un_agency",
            abstract="IOM migration-health page — UN-system coverage of migrant access to care, including NCDs.",
        ),
        official_document(
            key="undp:health",
            title="UNDP — Health",
            url="https://www.undp.org/health",
            year=2024,
            body="UNDP",
            family="un_agency",
            abstract="UNDP health programme page — development financing and health-system support for NCDs.",
        ),
        official_document(
            key="unep:pollution",
            title="UNEP — Chemicals and pollution action",
            url="https://www.unep.org/topics/chemicals-and-pollution-action",
            year=2024,
            body="UNEP",
            family="un_agency",
            abstract="UNEP chemicals and pollution topic page — environmental determinants of cardiovascular risk.",
        ),
        official_document(
            key="who:uhc-topic",
            ...
            body="World Health Organization",
            family="un_agency",
            abstract="WHO UHC topic hub — access to essential CVD medicines and primary-care packages.",
        ),
        official_document(
            key="who:uhc-factsheet",
            title="Universal health coverage (UHC) fact sheet",
            url="https://www.who.int/news-room/fact-sheets/detail/universal-health-coverage-(uhc)",
            year=2023,
            body="World Health Organization",
            family="un_agency",
            abstract="WHO fact sheet on universal health coverage, essential medicines, and financial protection.",
        ),
        official_document(
            key="who:phc",
            title="Primary health care — WHO health topic",
            url="https://www.who.int/health-topics/primary-health-care",
            year=2024,
            body="World Health Organization",
            family="un_agency",
            abstract="WHO primary-health-care topic page — the delivery setting for HEARTS and NCD packages.",
        ),
        official_document(
            key="who:pen",
            ...
            family="international_guideline",
            abstract=(
                "WHO PEN package of essential NCD interventions for primary care, including "
                "cardiovascular risk management in resource-constrained settings."
            ),
        ),
        official_document(
            key="nice:ta388",
            ...
            abstract=(
                "NICE technology appraisal TA388 for sacubitril/valsartan in symptomatic "
                "HFrEF — the English reimbursement counterpart to NG106."
            ),
        ),
        official_document(
            key="nhlbi:hf",
            title="Heart Failure — NHLBI",
            url="https://www.nhlbi.nih.gov/health/heart-failure",
            year=2024,
            body="National Heart, Lung, and Blood Institute",
            family="national_guideline",
            abstract="US NHLBI patient and clinician heart-failure information hub.",
        ),
        official_document(
            key="cdc:hf",
            title="About Heart Failure — CDC",
            url="https://www.cdc.gov/heart-disease/about/heart-failure.html",
            year=2024,
            body="US Centers for Disease Control and Prevention",
            family="national_guideline",
            abstract=(
                "CDC heart-failure topic page covering burden, risk factors, and "
                "prevention in the US public-health system."
            ),
        ),
        official_document(
            key="who:global-hearts",
            title="WHO Global Hearts Initiative",
            url="https://www.who.int/news-room/spotlight/global-hearts-initiative",
            year=2024,
            body="World Health Organization",
            family="un_agency",
            abstract=(
                "WHO Global Hearts initiative spotlight — the umbrella for HEARTS "
                "and related CVD packages in primary care."
            ),
        ),
    ]
...
def india_official_seeds() -> list[EvidenceRecord]:
    ...
    return [
        ...
        official_document(
            ...
            year=2024,
            body="Central Drugs Standard Control Organisation (India)",
            family="national_guideline",
            abstract=(
                "CDSCO official portal — India's national medicines regulator, relevant "
                "to HFrEF product registration and labelling."
            ),
        ),
        official_document(
            key="india:csi",
            title="Cardiological Society of India",
            url="https://www.csi.org.in/",
            year=2024,
            body="Cardiological Society of India",
            family="national_guideline",
            abstract="CSI official site — India's national cardiology society and position-statement host.",
        ),
    ]
