            ...
            "meta": {
                ...
                "time_savings": {
                    ...
                    "how": (
                        ...
                        "fixed-effect and DerSimonian-Laird random-effects summaries of "
                        "parsed CIs, leave-one-out, funnel points, and a citation-numbered "
            ...
            if q["id"] == "qualitative" and q.get("europe_pmc"):
                ...
                jobs.append(
                    (
                        "pubmed_qualitative",
                        q["id"],
                        lambda query=q["europe_pmc"]: connectors.pubmed(
                            query, min(n, 6)
                        ),
                    )
                )
...
def _omit_scanner_blocked(payload: dict) -> dict:
    ...
    if pooled:
        ...
        loo = [
            row
            for row in (pooled.get("leave_one_out") or [])
            if row.get("citation_id") not in drop_ids
        ]
        if funnel:
            pooled["funnel"] = funnel
            pooled["n_trials"] = len(funnel)
            pooled["leave_one_out"] = loo
        else:
