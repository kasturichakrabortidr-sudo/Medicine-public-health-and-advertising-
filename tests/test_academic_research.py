...
from academic_research.analyze import forest_rows, qualitative, quantitative, pooled_effects
...
class ExtractTests(unittest.TestCase):
    ...
    def test_does_not_treat_english_or_as_odds_ratio(self):
        text = (
            "The primary outcome was a composite of worsening heart failure or "
            "death from cardiovascular causes. The hazard ratio was 0.74 "
            "(95% CI, 0.65 to 0.85)."
        )
        effects = parse_effects(text)
        self.assertTrue(effects)
        self.assertEqual(effects[0].metric, "HR")
        self.assertEqual(effects[0].value, 0.74)
...
class AnalyzeTests(unittest.TestCase):
    ...
    def test_pooled_inverse_variance_from_parsed_cis(self):
        a = rec(
            title="Angiotensin-Neprilysin Inhibition versus Enalapril in Heart Failure",
            abstract=PARADIGM_ABSTRACT,
            citation_id=1,
        ...
        b.effects = parse_effects(b.abstract, b.title)
        rows = forest_rows([a, b])
        pooled = pooled_effects(rows)
        self.assertIsNotNone(pooled)
        self.assertEqual(pooled["n_trials"], 2)
        self.assertGreater(pooled["value"], 0.74)
        self.assertLess(pooled["value"], 0.80)
        self.assertEqual(len(pooled["funnel"]), 2)
        or_row = {
            "citation_id": 9,
            "label": "MITRA-FR (2018)",
            "metric": "OR",
        ...
        pooled_hr_only = pooled_effects(rows + [or_row])
        self.assertEqual(pooled_hr_only["n_trials"], 2)
        self.assertIn("never invented", pooled["method"])
        self.assertIsNotNone(pooled["random_effect"])
        self.assertEqual(pooled["random_effect"]["n_trials"], 2)
        self.assertGreaterEqual(pooled["random_effect"]["tau_squared"], 0.0)
        self.assertEqual(len(pooled["leave_one_out"]), 2)
        omitted = {row["omit"] for row in pooled["leave_one_out"]}
        self.assertTrue(any("PARADIGM" in (x or "") for x in omitted))
        self.assertTrue(any("DAPA-HF" in (x or "") for x in omitted))
...
class ScreeningTests(unittest.TestCase):
    ...
    def test_doi_seeds_skip_placeholder_topic_filter(self):
        ...
        self.assertIn("10.1056/NEJMoa1805374", hf_seeds)
        self.assertIn("10.1016/S0140-6736(21)01754-2", hf_seeds)
        self.assertIn("10.1016/j.cjca.2021.01.017", hf_seeds)
        self.assertIn("10.1002/ejhf.2115", hf_seeds)
        self.assertIn("10.1371/journal.pone.0310600", hf_seeds)
        self.assertIn("10.1001/jama.2022.23924", hf_seeds)
        self.assertIn("10.1016/j.jacc.2023.12.024", hf_seeds)
        self.assertIn("10.1002/ejhf.3215", hf_seeds)
    ...
    def test_official_url_seeds_are_allowlisted_and_skip_screen(self):
        ...
        self.assertTrue(_allowlisted("https://www.unicef.org/health/non-communicable-diseases"))
        self.assertTrue(_allowlisted("https://www.ohchr.org/en/health"))
        self.assertTrue(_allowlisted("https://www.nice.org.uk/guidance/ta388"))
        self.assertTrue(_allowlisted("https://www.csi.org.in/"))
        self.assertTrue(_allowlisted("https://cdsco.gov.in/opencms/opencms/en/Home/"))
        self.assertTrue(_allowlisted("https://www.itu.int/"))
        self.assertTrue(_allowlisted("https://www.cdc.gov/heart-disease/about/heart-failure.html"))
        self.assertTrue(_allowlisted("https://www.unhcr.org/what-we-do/protect-human-rights/public-health"))
        self.assertTrue(_allowlisted("https://www.iom.int/migration-health"))
        self.assertTrue(_allowlisted("https://www.undp.org/health"))
        ...
        self.assertIn("https://www.unicef.org/health/non-communicable-diseases", urls)
        self.assertIn("https://www.ohchr.org/en/health", urls)
        self.assertIn("https://www.nice.org.uk/guidance/ta388", urls)
        self.assertIn("https://www.csi.org.in/", urls)
        self.assertIn("https://www.who.int/publications/i/item/9789240009226", urls)
        self.assertIn("https://www.cdc.gov/heart-disease/about/heart-failure.html", urls)
        self.assertIn("https://www.unhcr.org/what-we-do/protect-human-rights/public-health", urls)
        self.assertIn("https://www.iom.int/migration-health", urls)
        self.assertIn("https://www.undp.org/health", urls)
        self.assertGreaterEqual(len(seeds), 52)
        seed = rec(
