...
class DemoDeckTests(unittest.TestCase):
    ...
    def test_has_quant_qual_and_visuals(self):
        ...
        self.assertTrue(str(self.deck["meta"]["pipeline_version"]).startswith("1."))
        self.assertGreaterEqual(len(self.deck.get("quantitative", {}).get("geography") or []), 1)
        ...
        self.assertTrue(self.deck["quantitative"].get("pooled_effect"))
        self.assertGreaterEqual(
            (self.deck["quantitative"].get("pooled_effect") or {}).get("n_trials") or 0, 1
        )
