...
def test_trivora_deck_is_a_strategy_argument_not_an_abstract_dump():
    ...
    assert not close["title"].lower().endswith(" is.")
    assert "and not." not in (close.get("callout") or {}).get("text", "").lower()
    title = next(s for s in slides if s["id"] == "title")
    bet_card = next(c for c in title["cards"] if c.get("meta") == "The bet")
    assert "step-up" in bet_card["title"].lower()
